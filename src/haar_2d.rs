...
pub fn inverse(input: &Vec<f64>) -> Vec<f64> {
    let mut output: Vec<f64> = Vec::new();
    let mut output_temp: Vec<f64> = vec![0.0; 64];
    let mut inverse_input = Vec::new();

    for x in 0..8 {
        for y in 0..8 {
            inverse_input.push(input[x + 8 * y]);
        }

        let inverse = haar_1d::inverse(&inverse_input);

        output.extend(inverse.iter().cloned());
    ...
    for x in 0..8 {
        for y in 0..8 {
            output_temp[x + 8 * y] = output[x * 8 + y];
        }
    }

    output = Vec::new();

    for chunks in &output_temp.iter().chunks(8) {
        inverse_input = haar_1d::inverse(&chunks.map(|x| *x).collect_vec());

        output.extend(inverse_input.iter().cloned());
    }
