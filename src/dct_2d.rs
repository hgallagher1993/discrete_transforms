...
pub fn forward(input: &Vec<f64>) -> Vec<f64> {
    ...
    for chunks in &input.iter().chunks(8) {
        forward_input = dct_1d::forward(&chunks.map(|x| *x).collect_vec());

        coefficients.extend(forward_input.iter().cloned());
    }
    ...
    for x in 0..8 {
        for y in 0..8 {
            forward_input.push(coefficients[x + 8 * y]);
        }

        output_temp.extend(dct_1d::forward(&forward_input).iter().cloned());
    ...
    for x in 0..8 {
        for y in 0..8 {
            coefficients[x + 8 * y] = output_temp[x * 8 + y];
        }
    }
...
pub fn inverse(input: &Vec<f64>) -> Vec<f64> {
    let mut pixel_intensities: Vec<f64> = Vec::new();
    ...
    for x in 0..8 {
        ...
        let inverse = dct_1d::inverse(&inverse_input);

        pixel_intensities.extend(inverse.iter().cloned());
    ...
    for x in 0..8 {
        for y in 0..8 {
            output_temp[x + 8 * y] = pixel_intensities[x * 8 + y];
        }
    }

    pixel_intensities = Vec::new();

    for chunks in &output_temp.iter().chunks(8) {
        inverse_input = dct_1d::inverse(&chunks.map(|x| *x).collect_vec());

        pixel_intensities.extend(inverse_input.iter().cloned());
    }
